...
namespace mewt::ext::sdl
{
	...
	texture_t::texture_t(const renderer_t& renderer, const texture_config_t& config)
		 : super_t(checkPointer(SDL_CreateTexture(renderer.get(), (int)config._format.get(), (int)config._access, config._size._width.get(), config._size._height.get())))
	{
	}
	...
	texture_config_t texture_t::get_config() const
	{
		...
		int width;
		int height;
		checkStatus(SDL_QueryTexture(get(), &format, &access, &width, &height));
